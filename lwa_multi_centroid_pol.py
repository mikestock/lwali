...
if __name__ == '__main__':
    ...
    frames = []
    frames.append( inputFile[ 'dirty00' ] )
    frames.append( inputFile[ 'dirty11' ] )
    frames.append( inputFile[ 'dirty10' ] )
    frames.append( inputFile[ 'dirty01' ] )
    
    print ('loaded file has shape: %s'%repr( frames[0].shape ) )
    ...
    for key in frames[0].attrs.keys():
        setattr( settings, key, frames[0].attrs[key])

    NFrames = (settings.stopsample-settings.startsample)//settings.steptime
    NImage  = settings.imagesize
    print (NFrames, NImage, frames[0].shape)
    ...
    #convert to pixels
    dca = (frames[0].attrs['bbox'][0][1]-frames[0].attrs['bbox'][0][0])/frames[0].attrs['imagesize']
    ...
    while iFrame < NFrames:
        ...
        I = frames[0][iFrame] + frames[1][iFrame]
        Q = frames[0][iFrame] - frames[1][iFrame]
        U = frames[2][iFrame] + frames[3][iFrame]
        V = frames[2][iFrame] - frames[3][iFrame]
        ...
        iCentroids = 0
        cbmax = 0
        while iCentroids < MAX_CENTROIDS_PER_IMAGE and im.max() > I.max()/30:
            iCentroids += 1
            ...
            #get the other 3 stokes parameters
            stokesI = I[round(i),round(j)]
            stokesQ = Q[round(i),round(j)]
            stokesU = U[round(i),round(j)]
            stokesV = V[round(i),round(j)]

            centroids.append( [tSample, ca, cb, brightness, r, iCentroids, stokesI, stokesQ, stokesU, stokesV ] )
    ...
    outputDset.attrs['header'] = ['tSample', 'alpha', 'beta', 'brightness', 'residual', 'order', 'stokesI', 'stokesQ', 'stokesU', 'stokesV' ]

    #copy over the attributes from the dirty file to the centroid file
    for k in inputFile.attrs.keys():
        v = inputFile.attrs[k]
        outputFile.attrs[k] = v
    for k in frames[0].attrs.keys():
        v = frames[0].attrs[k]
